...
const debug = createDebug("react-informal");
...
export class Form extends React.Component {
  ...
  update = updater => {
    ...
    debug("Form state updated %o", this.form);
  ...
  submit = async performSubmission => {
    debug("Attempting to submit form");

    if (this.form.valid) {
      this.update(transforms.onFormSubmit());
      try {
        const result = await performSubmission(
          transforms.getFormValues(this.form)
        );
    ...
    debug("Form was not submitted due to invalid fields");
...
export const connectField = (mapField = field => ({ field })) => Component => {
  ...
  return class FieldState extends React.Component {
    ...
    componentWillMount() {
      ...
      debug("Adding field `%s` to form", name);
    ...
    componentWillUnmount() {
      ...
      debug(`Removing field "${name}" from form`);
